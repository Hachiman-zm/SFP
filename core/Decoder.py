...
class Synthesis_mv_net(nn.Module):
    ...
    def __init__(self):
        ...
        self.deconv1 = nn.ConvTranspose2d(out_channel_M, out_channel_N, 3, stride=2, padding=1, output_padding=1)
        torch.nn.init.xavier_normal_(self.deconv1.weight.data, math.sqrt(2 * 1))
        torch.nn.init.constant_(self.deconv1.bias.data, 0.01)
        self.relu1 = nn.LeakyReLU(negative_slope=0.1)
        self.deconv2 = nn.Conv2d(out_channel_N, out_channel_N, 3, stride=1, padding=1)
        ...
        self.deconv4 = nn.Conv2d(out_channel_N, out_channel_N, 3, stride=1, padding=1)
        ...
        self.deconv5 = nn.ConvTranspose2d(out_channel_N, out_channel_N, 3, stride=2, padding=1, output_padding=1)
        ...
        self.deconv6 = nn.Conv2d(out_channel_N, out_channel_N, 3, stride=1, padding=1)
        ...
        self.deconv7 = nn.ConvTranspose2d(out_channel_N, out_channel_N, 3, stride=2, padding=1, output_padding=1)
        ...
        self.deconv8 = nn.Conv2d(out_channel_N, 2, 3, stride=1, padding=1)
        torch.nn.init.xavier_normal_(self.deconv8.weight.data,
                                     (math.sqrt(2 * 1 * (out_channel_N + 2) / (out_channel_N + out_channel_N))))
